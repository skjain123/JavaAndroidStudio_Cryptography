...
public class Cryptography {
	...
	private String de_Caesar(String input, int key) {
		...
		for (int i = 0; i < input.length(); i++) {
			if (input.charAt(i) >= ul && input.charAt(i) <= uh && input.charAt(i) - key < ul) { //upper
				output += (char) (uh - ul + (input.charAt(i)));
			} else if (input.charAt(i) >= ll && input.charAt(i) <= lh && input.charAt(i) - key < ll) { //lower
				output += (char) (lh - ll + (input.charAt(i)));
			} else {
				output += (char) (input.charAt(i) - key);
			}
		}
